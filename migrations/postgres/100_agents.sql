...
CREATE TABLE IF NOT EXISTS agents (
    ...
    tools        TEXT[] NOT NULL DEFAULT '{extract_pdf,extract_docx,extract_xlsx,extract_text_file,wiki_create_page,wiki_edit_page,wiki_append_section,wiki_update_frontmatter,wiki_rebuild_index,wiki_search,wiki_read_page,wiki_grep,wiki_list_pages,wiki_check_duplicate,wiki_propose_new,wiki_propose_edit,wiki_propose_append,wiki_propose_multi_edit,feedback_record}',
...
-- Existing DBs created by an earlier version of this file lack `description`; the
-- seed INSERT below references it, so add it here (before the INSERT) to keep this
-- migration self-consistent. 103 backfills descriptions on already-seeded rows.
ALTER TABLE agents ADD COLUMN IF NOT EXISTS description TEXT NOT NULL DEFAULT '';

INSERT INTO agents (id, display_name, identity, description, accent, theme_base, schema_kind, modes, tools,
...
VALUES
  ('conwo', 'Conwo',
   ...
   'Product, configuration, and debugging answers for WorkInSync.',
  ...
  ('infosec', 'Infosec',
   ...
   'Information-security questions from the organization''s security knowledge base.',
   '#a78bfa', 'dark', 'generic', '{api}',
   '{extract_pdf,extract_docx,extract_xlsx,extract_text_file,wiki_create_page,wiki_edit_page,wiki_append_section,wiki_update_frontmatter,wiki_rebuild_index,wiki_search,wiki_read_page,wiki_grep,wiki_list_pages,wiki_check_duplicate,wiki_propose_new,wiki_propose_edit,wiki_propose_append,wiki_propose_multi_edit,feedback_record}',
